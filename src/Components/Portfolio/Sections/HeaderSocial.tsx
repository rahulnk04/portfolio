...
const HeaderSocial = (props: Props) => {
  ...
  const { width = 44, height = 44, autoArrange } = props;
  const { social } = Data;
  ...
  return (
    <Box
      sx={{
        ...
        alignItems: 'center',
        gap: 1.5,
        //autoArrange then fit with space available and move to next line if needed, else keep in one line and overflow
        flexWrap: autoArrange ? 'wrap' : 'nowrap', // did not work in mobile view
      }}
    >
      <Grid container spacing={{ xs: 0, md: 1 }} columns={social.length}>
        {social.map((link: SocialLink, i: number) => (
          <Grid size={{ xs: 2, md: 1, sm: 1 }} key={i}>
